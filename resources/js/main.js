...
(function () {
  ...
  function makeModalEditable(mod) {
    ...
    if (ridWrap && !ridWrap.querySelector('.modal-rid-regen-btn')) {
  ...
  function makeDownloadLinksEditable(mod) {
    ...
    sections.forEach(function (section, sectionIdx) {
      ...
      addBtn.className = 'modal-edit-add-btn';
      addBtn.textContent = '+ 添加链接';
      addBtn.style.marginTop = '8px';
      addBtn.addEventListener('click', function () {
        var url = prompt('请输入下载链接URL：');
        if (!url) return;
        var text = prompt('请输入链接显示文字：', '下载链接');
        if (!text) text = '下载链接';
        if (!mod.downloadLinks) mod.downloadLinks = [];
        mod.downloadLinks.push({ text: text, url: url });
        saveEditData();
        oM(mod);
        showToast('链接已添加');
      });
      ...
      items.forEach(function (item) {
        var delBtn = document.createElement('button');
        delBtn.className = 'modal-link-edit-delete';
        delBtn.innerHTML = '×';
        delBtn.title = '删除链接';
        delBtn.addEventListener('click', function () {
          if (confirm('确定要删除这个下载链接吗？')) {
            var linkText = item.querySelector('.dn a');
            if (linkText) {
              var text = linkText.textContent.trim();
              mod.downloadLinks = mod.downloadLinks.filter(function (l) { return l.text !== text; });
              saveEditData();
              oM(mod);
              showToast('链接已删除');
            }
  ...
  function makeAuthorsEditable(mod) {
    ...
    addBtn.style.marginTop = '8px';
    addBtn.addEventListener('click', function () {
      var name = prompt('请输入作者名称：');
      if (!name) return;
      var role = prompt('请输入作者角色（可选）：', '');
      if (!mod.author) mod.author = '';
      if (mod.author) mod.author += ' ';
      if (role) mod.author += '[' + role + '] - ' + name;
      else mod.author += name;
      saveEditData();
      oM(mod);
      showToast('作者已添加');
    });
    authorSection.appendChild(addBtn);

    var authorRows = authorSection.querySelectorAll('.ar');
    authorRows.forEach(function (row) {
      ...
      delBtn.addEventListener('click', function () {
        if (confirm('确定要删除这个作者吗？')) {
          var nameEl = row.querySelector('.an');
          if (nameEl) {
            var name = nameEl.textContent.trim();
            var authors = pA(mod.author || '');
            authors = authors.filter(function (a) { return a.name !== name; });
  ...
  function makePreviewsEditable(mod) {
    var previewSection = mC.querySelector('.pp');
    if (!previewSection) return;

    var isImages = !!mod.previewImages && mod.previewImages.length > 0;
    var isVideos = !!mod.previewVideos && mod.previewVideos.length > 0;

    var addImgBtn = document.createElement('button');
    addImgBtn.className = 'modal-edit-add-btn';
    addImgBtn.textContent = '+ 添加图片URL';
    ...
    if (isImages || !isVideos) btnWrap.appendChild(addImgBtn);
    if (isVideos) btnWrap.appendChild(addVidBtn);
    previewSection.parentElement.appendChild(btnWrap);

    // 为每个预览添加删除按钮
    var previewItems = previewSection.querySelectorAll('.pii, .pvi');
    previewItems.forEach(function (item) {
      ...
      delBtn.addEventListener('click', function () {
        if (confirm('确定要删除这个预览吗？')) {
          var url = item.src || item.getAttribute('src');
          if (item.classList.contains('pii')) {
            mod.previewImages = (mod.previewImages || []).filter(function (u) { return u !== url; });
          } else {
            mod.previewVideos = (mod.previewVideos || []).filter(function (u) { return u !== url; });
          }
          saveEditData();
          oM(mod);
          showToast('预览已删除');
