(function () {
  ...
  function makeDownloadLinksEditable(mod) {
    ...
    saveBtn.addEventListener('click', function () {
      ...
      if (latestCat && latestCat.items.length > 0) {
        ...
        if (first.version) mod.badge = first.version;
        if (first.size) mod.size = first.size;
        if (first.date) mod.date = first.date;
      }
      saveEditData();
      oM(mod);
      showToast('下载方式已保存');
  ...
  function makeAuthorsEditable(mod) {
    ...
    var slLabels = mC.querySelectorAll('.sl');
    slLabels.forEach(function (sl) {
      if (sl.textContent.trim() === '作者') sl.style.display = 'none';
    });
    ...
    function renderCards() {
      ...
      saveBtn.addEventListener('click', function () {
        ...
        newAuthors.forEach(function (a) {
          if (a.links && a.links.length) {
            a.links.forEach(function (l) {
              mod.authorLinks.push({ text: l.text + '|' + a.name, url: l.url });
            });
          }
        });
        saveEditData();
  ...
  function makePreviewsEditable(mod) {
    ...
    psgElements.forEach(function (el) { el.style.display = 'none'; });
    ppElements.forEach(function (el) { el.style.display = 'none'; });
    peElements.forEach(function (el) { el.style.display = 'none'; });
    psgBtns.forEach(function (el) { el.style.display = 'none'; });
    psElements.forEach(function (el) { el.style.display = 'none'; });
    if (pAEl) pAEl.style.display = 'none';
    var slLabels = mC.querySelectorAll('.sl');
    slLabels.forEach(function (sl) {
      if (sl.textContent.trim() === '预览') sl.style.display = 'none';
    });
    var editWrap = document.createElement('div');
    editWrap.className = 'inline-edit-wrap';
