...
(function () {
  ...
  function makeModalEditable(mod) {
    ...
    if (ridWrap && !ridWrap.querySelector('.modal-rid-input')) {
      // 清空原有RID显示，替换为输入框
      var ridSpan = ridWrap.querySelector('.mr');
      if (ridSpan) {
        ridSpan.innerHTML = '<span class="mrt">RID </span><input type="text" class="modal-rid-input editable-field" value="' + esc(mod.id) + '" style="width:120px;padding:2px 6px;font-size:.85rem;border:2px solid var(--bd);border-radius:6px;background:var(--bg);color:var(--tp);font-family:inherit">';
        var ridInput = ridSpan.querySelector('.modal-rid-input');
        ridInput.addEventListener('blur', function () {
          var newId = this.value.trim();
          if (newId && newId !== mod.id) {
            // 检查ID是否重复
            var exists = modData.some(function (m) { return m.id === newId && m !== mod; });
            if (exists) {
              showToast('RID已存在，请使用其他ID');
              this.value = mod.id;
              return;
            }
            mod.id = newId;
      ...
      var ridRegenBtn = document.createElement('a');
      ridRegenBtn.className = 'mra';
      ridRegenBtn.innerHTML = '↻ 重新生成';
      ridRegenBtn.style.cursor = 'pointer';
    ...
    if (tagsWrap) {
      ...
      tagsWrap.querySelectorAll('.mtg').forEach(function (tagEl) {
        if (tagEl.querySelector('.tag-del-inline')) return;
        var delBtn = document.createElement('span');
        delBtn.className = 'tag-del-inline';
        delBtn.textContent = ' ×';
      ...
      // 添加标签按钮
      if (!tagsWrap.querySelector('.modal-edit-add-btn')) {
  ...
  function refreshModalTags(mod) {
    ...
    mod.tags.forEach(function (t) {
      h += '<span class="mtg ' + t.toLowerCase() + '">' + esc(t) + '<span class="tag-del-inline" style="cursor:pointer;color:#b14b4b;font-weight:700;margin-left:2px"> ×</span></span>';
    });
    h += '<button class="modal-edit-add-btn" style="margin-left:8px">+ 添加标签</button>';
    tagsWrap.innerHTML = h;

    // 重新绑定事件
    tagsWrap.querySelectorAll('.tag-del-inline').forEach(function (delBtn) {
      delBtn.addEventListener('click', function () {
        var tag = this.parentElement.textContent.replace(' ×', '').trim();
        if (confirm('确定删除标签 "' + tag + '" 吗？')) {
          removeTag(mod.id, tag);
          refreshModalTags(mod);
        }
    ...
    var addBtn = tagsWrap.querySelector('.modal-edit-add-btn');
    if (addBtn) {
  ...
  function makeDownloadLinksEditable(mod) {
    ...
    var sectionNames = ['latest', 'alternative', 'history'];
    var sectionLabels = ['最新版本', '其他下载方式', '历史版本'];

    sections.forEach(function (section, sectionIdx) {
      var sb = section.querySelector('.sb');
      if (!sb) return;

      // 清除之前的编辑控件
      var oldAddBtn = sb.querySelector('.dl-edit-add-btn');
      if (oldAddBtn) oldAddBtn.remove();

      // 添加新链接按钮
      var addBtn = document.createElement('button');
      addBtn.className = 'modal-edit-add-btn dl-edit-add-btn';
      addBtn.textContent = '+ 添加链接';
      addBtn.style.marginTop = '8px';
      addBtn.addEventListener('click', function () {
        openDownloadLinkEditor(mod, sectionNames[sectionIdx], null, function () {
          saveEditData();
          oM(mod);
          showToast('链接已添加');
        });
      ...
      // 为每个现有链接添加编辑/删除按钮
      var items = sb.querySelectorAll('.di');
      items.forEach(function (item, itemIdx) {
        // 清除之前的按钮
        var oldEdit = item.querySelector('.dl-edit-btn');
        if (oldEdit) oldEdit.remove();
        var oldDel = item.querySelector('.dl-delete-btn');
        if (oldDel) oldDel.remove();

        var linkData = cl[sectionNames[sectionIdx]][itemIdx];
        if (!linkData) return;
        ...
        editBtn.className = 'dl-edit-btn';
        editBtn.textContent = '编辑';
        editBtn.style.cssText = 'padding:2px 8px;font-size:.75rem;border-radius:6px;border:1px solid var(--bd);background:var(--bg);cursor:pointer;color:var(--tp);font-family:inherit';
        editBtn.addEventListener('click', function () {
          openDownloadLinkEditor(mod, sectionNames[sectionIdx], linkData, function () {
            saveEditData();
            oM(mod);
            showToast('链接已更新');
          });
        ...
        delBtn.style.cssText = 'padding:2px 8px;font-size:.75rem;border-radius:6px;border:1px solid #b14b4b;background:#fff0f0;cursor:pointer;color:#b14b4b;font-family:inherit';
        delBtn.addEventListener('click', function () {
          if (confirm('确定要删除这个下载链接吗？')) {
            mod.downloadLinks = mod.downloadLinks.filter(function (l) {
              return !(l.text === linkData.text && l.url === linkData.url);
            });
            saveEditData();
            oM(mod);
            showToast('链接已删除');
        ...
        item.querySelector('.dic').appendChild(btnWrap);
      });
    });
  }

  function openDownloadLinkEditor(mod, sectionType, linkData, callback) {
    var existing = document.getElementById('dlEditorOverlay');
    if (existing) existing.remove();

    var isEdit = !!linkData;
    var overlay = document.createElement('div');
    overlay.className = 'goto-popup-overlay active';
    overlay.id = 'dlEditorOverlay';
    ...
    var title = document.createElement('div');
    title.className = 'goto-popup-title';
    title.textContent = isEdit ? '编辑下载链接' : '添加下载链接';

    var form = document.createElement('div');
    form.style.cssText = 'display:flex;flex-direction:column;gap:12px';

    var fields = [
      { id: 'dlText', label: '显示文字', value: isEdit ? linkData.text : '', placeholder: '例如：百度网盘、夸克网盘' },
      { id: 'dlUrl', label: '链接URL', value: isEdit ? linkData.url : '', placeholder: 'https://...' },
      { id: 'dlDesc', label: '描述（可选）', value: isEdit ? (linkData.desc || '') : '', placeholder: '版本说明、提取码等' }
    ];

    fields.forEach(function (f) {
      var label = document.createElement('label');
      label.textContent = f.label;
      label.style.cssText = 'font-size:.85rem;color:var(--tm);font-weight:500';
      ...
      input.style.cssText = 'width:100%;padding:8px 12px;border:2px solid var(--bd);border-radius:8px;font-size:.9rem;font-family:inherit;background:var(--bg);color:var(--tp);box-sizing:border-box';
    ...
    cancelBtn.className = 'goto-popup-confirm';
    ...
    saveBtn.className = 'goto-popup-confirm';
    saveBtn.textContent = isEdit ? '保存' : '添加';
    saveBtn.addEventListener('click', function () {
      var text = document.getElementById('dlText').value.trim();
      var url = document.getElementById('dlUrl').value.trim();
      var desc = document.getElementById('dlDesc').value.trim();

      if (!text || !url) {
        showToast('请填写显示文字和链接URL');
        return;
      }
      ...
      if (isEdit) {
        // 更新现有链接
        var idx = mod.downloadLinks.findIndex(function (l) {
          return l.text === linkData.text && l.url === linkData.url;
        });
        if (idx !== -1) {
          mod.downloadLinks[idx] = { text: text, url: url, desc: desc || undefined };
        }
      } else {
        // 添加新链接，根据sectionType决定分类
        var newLink = { text: text, url: url };
        if (desc) newLink.desc = desc;

        // 根据sectionType设置特定的文字标记来分类
        if (sectionType === 'history') {
          if (!text.includes('历史版本') && !text.includes('旧版') && !text.includes('兼容') && !text.includes('备选')) {
            newLink.text = text + ' [历史版本]';
          }
        } else if (sectionType === 'alternative') {
          if (!text.includes('在线解析') && !text.includes('N网') && !text.includes('官方帖子')) {
            // 保持原样，由cLL函数根据关键词分类
          }
        }
        mod.downloadLinks.push(newLink);
      }
    ...
    setTimeout(function () {
      document.getElementById('dlText').focus();
    }, 100);
  }
  ...
  function makeAuthorsEditable(mod) {
    ...
    // 保存原始作者数据
    var authors = pA(mod.author || '');
    var authorLinks = mod.authorLinks || [];

    // 清空并重建为编辑界面
    authorSection.innerHTML = '';
    authorSection.style.cssText = 'display:flex;flex-direction:column;gap:10px';

    // 作者卡片列表
    var authorsList = document.createElement('div');
    authorsList.className = 'authors-edit-list';
    authorsList.style.cssText = 'display:flex;flex-direction:column;gap:10px';

    function renderAuthorCards() {
      authorsList.innerHTML = '';
      authors.forEach(function (author, idx) {
        var card = document.createElement('div');
        card.style.cssText = 'background:var(--bg);border:2px solid var(--bd);border-radius:12px;padding:12px;display:flex;flex-direction:column;gap:8px';
        ...
        roleInput.type = 'text';
        roleInput.placeholder = '岗位/别名';
        roleInput.value = author.role || '';
        roleInput.style.cssText = 'flex:0 0 100px;padding:6px 10px;border:2px solid var(--bd);border-radius:8px;font-size:.85rem;font-family:inherit;background:var(--bg);color:var(--tp)';
        roleInput.addEventListener('blur', function () {
          authors[idx].role = this.value.trim();
          updateAuthorData();
        });
        ...
        nameInput.style.cssText = 'flex:1;padding:6px 10px;border:2px solid var(--bd);border-radius:8px;font-size:.85rem;font-family:inherit;background:var(--bg);color:var(--tp)';
        nameInput.addEventListener('blur', function () {
          authors[idx].name = this.value.trim();
          updateAuthorData();
        });

        var delBtn = document.createElement('button');
        delBtn.innerHTML = '×';
        delBtn.style.cssText = 'width:32px;height:32px;border-radius:8px;border:1px solid #b14b4b;background:#fff0f0;color:#b14b4b;cursor:pointer;font-size:1rem;font-family:inherit';
        delBtn.addEventListener('click', function () {
          if (confirm('确定删除这位作者吗？')) {
        ...
        // 作者链接
        var authorLinkList = authorLinks.filter(function (l) {
          var ln = eNL(l.text);
          return ln && author.name && (ln.indexOf(author.name) !== -1 || author.name.indexOf(ln) !== -1);
        });

        var linksWrap = document.createElement('div');
        linksWrap.style.cssText = 'display:flex;flex-direction:column;gap:6px;margin-top:4px';

        authorLinkList.forEach(function (link, linkIdx) {
          var linkRow = document.createElement('div');
          linkRow.style.cssText = 'display:flex;gap:8px;align-items:center';

          var linkTextInput = document.createElement('input');
          linkTextInput.type = 'text';
          linkTextInput.placeholder = '平台名称';
          linkTextInput.value = link.text || '';
          linkTextInput.style.cssText = 'flex:0 0 120px;padding:4px 8px;border:2px solid var(--bd);border-radius:6px;font-size:.8rem;font-family:inherit;background:var(--bg);color:var(--tp)';
          linkTextInput.addEventListener('blur', function () {
            link.text = this.value.trim();
            saveEditData();
          });

          var linkUrlInput = document.createElement('input');
          linkUrlInput.type = 'text';
          linkUrlInput.placeholder = '链接URL';
          linkUrlInput.value = link.url || '';
          linkUrlInput.style.cssText = 'flex:1;padding:4px 8px;border:2px solid var(--bd);border-radius:6px;font-size:.8rem;font-family:inherit;background:var(--bg);color:var(--tp)';
          linkUrlInput.addEventListener('blur', function () {
            link.url = this.value.trim();
            saveEditData();
          });

          var linkDelBtn = document.createElement('button');
          linkDelBtn.innerHTML = '×';
          linkDelBtn.style.cssText = 'width:26px;height:26px;border-radius:6px;border:1px solid #b14b4b;background:#fff0f0;color:#b14b4b;cursor:pointer;font-size:.8rem;font-family:inherit';
          linkDelBtn.addEventListener('click', function () {
            var globalIdx = authorLinks.indexOf(link);
            if (globalIdx !== -1) {
              authorLinks.splice(globalIdx, 1);
              updateAuthorData();
              makeAuthorsEditable(mod);
              showToast('链接已删除');
            }
          ...
          linkRow.appendChild(linkTextInput);
          linkRow.appendChild(linkUrlInput);
        ...
        addLinkBtn.className = 'modal-edit-add-btn';
        addLinkBtn.textContent = '+ 添加平台链接';
        addLinkBtn.style.cssText = 'padding:4px 10px;font-size:.75rem;margin-top:4px;align-self:flex-start';
        addLinkBtn.addEventListener('click', function () {
          var newLink = { text: author.name + '｜', url: '' };
          authorLinks.push(newLink);
          updateAuthorData();
          makeAuthorsEditable(mod);
        });
    ...
    // 添加作者按钮
    var addAuthorBtn = document.createElement('button');
    addAuthorBtn.className = 'modal-edit-add-btn';
    addAuthorBtn.textContent = '+ 添加作者';
    addAuthorBtn.style.cssText = 'align-self:flex-start';
    addAuthorBtn.addEventListener('click', function () {
      authors.push({ role: '', name: '' });
      updateAuthorData();
      makeAuthorsEditable(mod);
    });
    authorSection.appendChild(addAuthorBtn);

    function updateAuthorData() {
      // 更新mod.author
      mod.author = authors.map(function (a) {
        if (a.role && a.name) return '[' + a.role + '] - ' + a.name;
        return a.name || '';
      }).join(' ');

      mod.authorLinks = authorLinks;
      saveEditData();
    }
  }

  function makePreviewsEditable(mod) {
    var previewContainer = mC.querySelector('#pA') || mC.querySelector('.pp');
    ...
    // 检查是否已经有编辑区域
    var existingEdit = parent.querySelector('.preview-edit-area');
    if (existingEdit) existingEdit.remove();

    var editArea = document.createElement('div');
    editArea.className = 'preview-edit-area';
    editArea.style.cssText = 'display:flex;flex-direction:column;gap:16px';

    // 图片编辑区
    var imgSection = document.createElement('div');
    imgSection.innerHTML = '<div style="font-size:.9rem;font-weight:600;color:var(--tp);margin-bottom:8px">' + imS + ' 图片链接（每行一个）</div>';

    var imgTextarea = document.createElement('textarea');
    imgTextarea.className = 'editable-field';
    imgTextarea.style.cssText = 'width:100%;min-height:100px;padding:10px;font-size:.85rem;font-family:inherit;border:2px solid var(--bd);border-radius:10px;resize:vertical';
    imgTextarea.value = (mod.previewImages || []).join('\n');
    imgTextarea.addEventListener('blur', function () {
      var lines = this.value.split('\n').map(function (s) { return s.trim(); }).filter(function (s) { return s; });
      mod.previewImages = lines;
      saveEditData();
    });

    imgSection.appendChild(imgTextarea);

    var imgBtnWrap = document.createElement('div');
    imgBtnWrap.style.cssText = 'display:flex;gap:8px;margin-top:6px';

    var addImgBtn = document.createElement('button');
    addImgBtn.className = 'modal-edit-add-btn';
    addImgBtn.textContent = '+ 添加图片链接';
    addImgBtn.addEventListener('click', function () {
      var url = prompt('请输入图片URL：');
      if (!url) return;
      if (!mod.previewImages) mod.previewImages = [];
      mod.previewImages.push(url);
      imgTextarea.value = mod.previewImages.join('\n');
      saveEditData();
      showToast('图片已添加');
    });

    var clearImgBtn = document.createElement('button');
    clearImgBtn.className = 'modal-edit-add-btn';
    clearImgBtn.textContent = '清空图片';
    clearImgBtn.style.cssText = 'background:#fff0f0;color:#b14b4b;border-color:#b14b4b';
    clearImgBtn.addEventListener('click', function () {
      if (confirm('确定清空所有图片链接吗？')) {
        mod.previewImages = [];
        imgTextarea.value = '';
        saveEditData();
        showToast('图片已清空');
    ...
    imgBtnWrap.appendChild(addImgBtn);
    imgBtnWrap.appendChild(clearImgBtn);
    imgSection.appendChild(imgBtnWrap);

    // 视频编辑区
    var vidSection = document.createElement('div');
    vidSection.innerHTML = '<div style="font-size:.9rem;font-weight:600;color:var(--tp);margin-bottom:8px">' + viS + ' 视频链接（每行一个）</div>';

    var vidTextarea = document.createElement('textarea');
    vidTextarea.className = 'editable-field';
    vidTextarea.style.cssText = 'width:100%;min-height:100px;padding:10px;font-size:.85rem;font-family:inherit;border:2px solid var(--bd);border-radius:10px;resize:vertical';
    vidTextarea.value = (mod.previewVideos || []).join('\n');
    vidTextarea.addEventListener('blur', function () {
      var lines = this.value.split('\n').map(function (s) { return s.trim(); }).filter(function (s) { return s; });
      mod.previewVideos = lines;
      saveEditData();
    });

    vidSection.appendChild(vidTextarea);

    var vidBtnWrap = document.createElement('div');
    vidBtnWrap.style.cssText = 'display:flex;gap:8px;margin-top:6px';

    var addVidBtn = document.createElement('button');
    addVidBtn.className = 'modal-edit-add-btn';
    addVidBtn.textContent = '+ 添加视频链接';
    addVidBtn.addEventListener('click', function () {
      var url = prompt('请输入视频URL：');
      if (!url) return;
      if (!mod.previewVideos) mod.previewVideos = [];
      mod.previewVideos.push(url);
      vidTextarea.value = mod.previewVideos.join('\n');
      saveEditData();
      showToast('视频已添加');
    });

    var clearVidBtn = document.createElement('button');
    clearVidBtn.className = 'modal-edit-add-btn';
    clearVidBtn.textContent = '清空视频';
    clearVidBtn.style.cssText = 'background:#fff0f0;color:#b14b4b;border-color:#b14b4b';
    clearVidBtn.addEventListener('click', function () {
      if (confirm('确定清空所有视频链接吗？')) {
        mod.previewVideos = [];
        vidTextarea.value = '';
        saveEditData();
        showToast('视频已清空');
    ...
    vidBtnWrap.appendChild(addVidBtn);
    vidBtnWrap.appendChild(clearVidBtn);
    vidSection.appendChild(vidBtnWrap);

    editArea.appendChild(imgSection);
    editArea.appendChild(vidSection);

    // 插入到预览容器后面
    parent.insertBefore(editArea, previewContainer.nextSibling);
  }
  ...
  function changeCover(modId) {
    var mod = modData.find(function (m) { return m.id === modId; });
    if (!mod) return;
    var currentUrl = Array.isArray(mod.coverImage) ? (mod.coverImage[0] || '') : (mod.coverImage || '');
  ...
  function collapseAllSections() {
    document.querySelectorAll('.sb').forEach(function (sb) {
      // 强制收起：先设置maxHeight为当前高度，然后添加co类
      if (!sb.classList.contains('co')) {
